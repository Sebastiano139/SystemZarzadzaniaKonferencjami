        ...
        RETURN
            (
                ...
                SELECT CONCAT('Warsztat: ', WorkshopName,
                              ',Data: ', Date, ' - ', WorkshopReservations.NormalTickets, ' biletów normalnych') AS Name,
            ...
            (
                ...
                FROM dbo.Clients
                         ...
                         INNER JOIN dbo.Conferences
                                    ON Conferences.ConferenceID = Reservations.ConferenceID
                                        AND Conferences.ConferenceID = @ConferenceID
                WHERE dbo.Conferences.Canceled = 0 and Firstname is not null and Lastname is not null
            )
go
...
CREATE FUNCTION [dbo].[sf_GetConferenceDayID]
(
@conferenceID int,
@date date
)
            ...
            IF (@cityName is not null and @countryName is not null)
                BEGIN
                    ...
                    SET @cityID = (Select cityID
                                   From Cities
                                   Where CountryID = @countryID and CityName = @cityName)
                    IF (@cityID is null)
                        BEGIN
...
BEGIN
    BEGIN TRY
        BEGIN TRAN UpdateReservationDay
             IF ((select count(*) from ReservationDays join Reservations R2 on ReservationDays.ReservationID = R2.ReservationID join IndividualClients IC on R2.ClientID = IC.ClientID
                where ReservationDayID = @reservationDayID
                )>0)
                   BEGIN
